...
export class IflowStatusChecker extends BaseProviderChecker {
  async checkStatus(): Promise<StatusCheckResult> {
    try {
      /*
       * Check API endpoint directly since Iflow is a newer provider
       * and may not have a public status page yet
       */
      const apiEndpoint = 'https://apis.iflow.cn/v1/models';
      const apiStatus = await this.checkEndpoint(apiEndpoint);

      // Check their website as a secondary indicator
      const websiteStatus = await this.checkEndpoint('https://docs.iflow.cn/docs/');
