...
export class WisdomStatusChecker extends BaseProviderChecker {
  async checkStatus(): Promise<StatusCheckResult> {
    try {
      /*
       * Check API endpoint directly since Wisdom is a newer provider
       * and may not have a public status page yet
       */
      const apiEndpoint = 'https://wisdom-gate.juheapi.com/v1/models';
      const apiStatus = await this.checkEndpoint(apiEndpoint);

      // Check their website as a secondary indicator
      const websiteStatus = await this.checkEndpoint('https://wisdom-gate.juheapi.com');
      ...
      console.error('Error checking Wisdom status:', error);
